...
如果把10处的偏移量定为0000，则array代表的**数据**的偏移量就是0000，20处的是0001，30的是0002，40的是0003。
...
在这个数组中，每个**数据**的偏移量为2个字节。设array偏移量为0000，则2为0002，3为0004，4为0006。
...
DWORD数据的偏移量为4字节，即在某一数组中，设第一个**数据**的偏移量是0000，则第二个数据是0004，第三个是0008，以此类推。
...
QWORD和SQWORD为64位无符号或者有符号整数分配存储空间。QWORD范围是**0~2^64**，SQWORD的范围是**-2^32~2^32-1**。例：
...
Intel把一个**压缩的二进制编码的十进制**（BCD，Binary Coded Decimal）整数存放在一个十字节的包中。每个字节包含两个十进制数字。在低9个存储字节中，每半个字节存放一个十进制数字。最高字节中，最高位表示数字的符号位。如果最高字节为**80h**，则这个是数是负数；如果是**00h**，则为整数。TBYTE的范围是**-999,999,999,999,999,999~999,999,999,999,999,999**。例：
...
**注意：汇编器并不会自动进行数字转换。因此TBYTE的初始值必须为十六进制。**

例：

```assembly
var1 tbyte 8000000000001234h ;即十进制数字-1234
```

如果想用**实数**去初始化TBYTE变量，可以使用FLD指令和FBSTP指令。先用FLD指令将实数压入浮点寄存器堆栈，再用FBSTP指令转换为压缩BCD码，这个指令会将数值**向最近的整数取整**。例：
...
.code
fld var1
fbstp var2 ;此时var2的值为00000000000000000002h，即十进制数字2
```
...
REAL4定义4字节**单**精度浮点变量；REAL8定义8字节**双**精度浮点变量；REAL10定义10字节**扩展**精度浮点变量。

下表描述不同精度浮点数的范围和有效数字个数：

| 数据类型 |有效数字|近似范围|
| :--------: | :----:| :----:|
| REAL4 | 6 | 1.18E-38 ~ 3.4E38 |
| REAL8 | 15 | 2.23E-308 ~ 1.79E308 |
| REAL10 | 19 | 3.37E-4932 ~ 1.18E4932 |
...
x86处理器在内存中按照小端顺序存储和检索数据。最低有效字节存放在分配给该数据的第一个内存地址中，剩余字节存放在随后的连续内存位置中。例：

```assembly
var dword 123456h
```

变量var存放着一个十六进制数字12345678h，那么根据小端顺序，数字在内存中的存储方式应该是下面这样（假设第一个字节的地址为0x0000）：
...
.data? 伪指令声明未初始化的数据。当定义**大量**未初始化的数据的时候，这个伪指令将会大大减少程序的存储占用。例：

```assembly
.data?
array1 dword 2000 dup(?) ;数组申请2000x4字节未初始化的存储空间

.data
array2 dword 2000 dup(?) ;数组申请2000x4字节未初始化的存储空间，但是多出来2000x4字节的占用
```

### 代码和数据混合

汇编器允许在程序中进行数据和代码的来回切换。例：
